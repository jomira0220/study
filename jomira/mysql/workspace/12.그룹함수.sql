...
SELECT * FROM member WHERE member_gender = 'm';
...
# [조건] 소수점 이하 2자리까지 출력
SELECT ROUND(AVG(book_price),2) FROM book; # ROUND로 소수점 한자리까지만 노출

# [문제] book테이블에서 도서 가격을 10%로 인상했을 때의 총합 가져오기
SELECT SUM(book_price), SUM(book_price + book_price * 0.1) FROM book;
