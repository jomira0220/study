...
SELECT * FROM STUDENT WHERE STUDENT_HEIGHT < 165; 

-- 7. 1학년이고 동시에 키가 170 이상인 학생의 이름, 학년, 키, 몸무게 을 검색하라
SELECT STUDENT_NAME, STUDENT_GRADE, STUDENT_HEIGHT, STUDENT_WEIGHT FROM STUDENT WHERE STUDENT_HEIGHT >= 170;

-- 9. 몸무게 45보다 크거나 같고 50보다 작거나 같은 학생의 이름, 주소, 키, 몸무게을 검색 
SELECT STUDENT_NAME, STUDENT_GRADE, STUDENT_HEIGHT, STUDENT_WEIGHT FROM STUDENT WHERE STUDENT_HEIGHT >= 170;

-- 11. 학생의 번호, 이름, 학년을 검색하라
SELECT * FROM STUDENT WHERE STUDENT_NO = 6; # 학생번호 6번 박지은
SELECT * FROM STUDENT WHERE STUDENT_NAME = '김태희'; # 학생이름 김태희
SELECT * FROM STUDENT WHERE STUDENT_GRADE = 1; # 1학년 학생 전원

-- 13. 박씨 성을 가진 학생을 검색하라
SELECT * FROM STUDENT WHERE STUDENT_NAME LIKE '박%';

-- 14. 김씨, 이씨, 박씨 성을 가진 학생 모두를 검색하되 반드시 LIKE 연산자를 사용하라
SELECT * FROM STUDENT WHERE STUDENT_NAME LIKE '김%' OR STUDENT_NAME LIKE '이%' OR STUDENT_NAME LIKE '박%';
...
SELECT STUDENT_NAME, STUDENT_GRADE, STUDENT_ADDR, STUDENT_WEIGHT FROM STUDENT 
...
SELECT STUDENT_NO, STUDENT_NAME, STUDENT_ADDR FROM STUDENT ORDER BY STUDENT_NAME ASC;

-- 23. 학생 테이블에서 '보아' 학생과 학년이 동일한 모든 학생의 이름과 키, 몸무게를 검색하라
SELECT @BOA_GRADE := STUDENT_GRADE FROM STUDENT WHERE STUDENT_NAME = '보아';
 # 보아의 학년을 위에서 변수로 지정하고 해당 변수로 밑에서 검색
SELECT STUDENT_NAME, STUDENT_HEIGHT, STUDENT_WEIGHT FROM STUDENT WHERE STUDENT_GRADE = @BOA_GRADE;

-- 24. 1번 학과 학생들의 평균 키보다 작은 학생의 이름, 학년, 키를 검색하라
SELECT @STUDENT_HEIGHT_AVG := AVG(STUDENT_HEIGHT) FROM STUDENT; # 170.7778
SELECT STUDENT_NAME, STUDENT_GRADE, STUDENT_HEIGHT FROM STUDENT
WHERE STUDENT_MAJOR_NO = 1 AND STUDENT_HEIGHT < @STUDENT_HEIGHT_AVG; # 1번 학과이면서 170.7778 보다 작으면 출력

-- 25. '김태희' 학생과 학년이 같고, '김태희' 학생보다 큰 학생의 이름, 학년, 키를 검색
SELECT @GRADE_TA := STUDENT_GRADE FROM STUDENT WHERE STUDENT_NAME = '김태희'; # 김태희 학년 1
SELECT @HEIGHT_TA := STUDENT_HEIGHT FROM STUDENT WHERE STUDENT_NAME = '김태희'; # 김태희 키 165
SELECT STUDENT_NAME, STUDENT_GRADE, STUDENT_HEIGHT FROM STUDENT 
WHERE @GRADE_TA = STUDENT_GRADE AND @HEIGHT_TA < STUDENT_HEIGHT; # 태희와 같은 1학년 중에서 태희키 165보다 큰 학생 
...
SELECT AVG(SCORE) FROM COURSE_HISTORY WHERE SUBJECT_NO = 1;

-- 30. 과목별 평균 점수를 검색하라
SELECT SUBJECT_NO, AVG(SCORE) FROM COURSE_HISTORY GROUP BY SUBJECT_NO;
SELECT * FROM COURSE_HISTORY; # 과목별 점수 현황
SELECT * FROM MAJOR_SUBJECT; # 과목목록
